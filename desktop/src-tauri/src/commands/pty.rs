...
fn list_panes_raw(callsite: &'static str) -> Result<String, String> {
    let output = debug_spawn::run_logged(
        ...
        &[
            ...
            "-a",
            "-F",
            "#{pane_id}|CT|#{session_name}|CT|#{window_index}|CT|#{window_name}|CT|#{pane_width}|CT|#{pane_height}|CT|#{pane_current_command}|CT|#{window_id}",
        ],
        callsite,
    )
...
pub fn list_free_panes() -> Result<Vec<FreePaneInfo>, String> {
    ...
    for line in raw.lines() {
        let p: Vec<&str> = line.split("|CT|").collect();
        if p.len() < 7 {
            continue;
        }
        let session = p[1];

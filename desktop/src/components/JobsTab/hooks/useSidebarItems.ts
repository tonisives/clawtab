...
function isJobProcess(process: DetectedProcess, jobs: Job[], statuses: ReturnType<typeof useJobsCore>["statuses"]): boolean {
  const jobSlugs = new Set(jobs.map((job) => job.slug));
  if (process.matched_job && jobSlugs.has(process.matched_job)) return true;
  for (const job of jobs) {
    const status = statuses[job.slug];
    if (status?.state === "running" && status.pane_id === process.pane_id) return true;
  }
  ...
  return jobs.some((job) => (
    ...
    && (job.name.toLowerCase() === generatedNameLower || job.slug.toLowerCase().endsWith(`/${generatedNameLower}`))
  ));
}
  ...
  const orderedItems = useMemo(() => {
    ...
    for (const key of keys) {
      ...
      for (const process of allProcesses) {
        if (isJobProcess(process, jobs, core.statuses)) continue;
    ...
    return result;
  }, [core.jobs, core.processes, core.statuses, jobOrder, pendingProcess, shellPanes, sortMode, stoppingProcesses]);

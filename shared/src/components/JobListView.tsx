...
function displayGroupName(group: string, jobs: RemoteJob[]): string {
  if (group !== "default") return group;
  const fp = jobs[0]?.folder_path ?? jobs[0]?.work_dir;
  return fp ? fp.split("/").filter(Boolean).pop() ?? "General" : "General";
}

function matchesProcessQuery(process: DetectedProcess, query: string): boolean {
  return matchesQuery([
...
function matchesShellQuery(shell: ShellPane, query: string): boolean {

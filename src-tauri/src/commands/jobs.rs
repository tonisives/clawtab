...
/// Generate the CLAUDE.md for the agent bot directory (~/.config/clawtab/agent/).
...
pub fn generate_agent_claude_md(settings: &AppSettings, jobs: &[Job]) -> String {
    ...
    out.push_str("# ClawTab Agent Bot\n\n");
    out.push_str("You are the ClawTab agentic bot, controlled via Telegram.\n\n");
    ...
    if let Some(config_dir) = crate::config::config_dir() {
        out.push_str(&format!("\nYou also have access to ClawTab config at `{}`\n", config_dir.display()));
    }
    ...
    out.push_str("- Use cwtctl to interact with ClawTab jobs.\n");

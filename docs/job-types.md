...
Project-based AI agent using a `.cwdt/` directory structure.

```yaml
job_type: folder
folder_path: /projects/myapp/.cwdt
```

### .cwdt Directory Structure

```
myproject/
  .cwdt/
    job.md          # required -- agent instructions/prompt
    cwdt.md         # auto-generated -- agent context (jobs, secrets, tools)
...
**`cwdt.md`** -- Auto-generated by ClawdTab on job save, settings change, or app startup. Contains:
...
```bash
export SECRET_KEY='value' && cd /projects/myapp && claude $'@.cwdt/cwdt.md @.cwdt/job.md\n\n<job.md content>'
```

The job runs from the project root (parent of `.cwdt/`), not from inside `.cwdt/`.
